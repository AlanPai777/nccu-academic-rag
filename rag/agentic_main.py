...
**表單編號規則**：如果get_page的回傳裡出現「[偵測到表單編號: ...]」這個標記，代表頁面提到官方表單。若問題需要完整辦理流程細節（例如逐一列出蓋章站點、審核層級，不只是概略說明），該呼叫get_form(form_id=...)取得表單本身的完整內容——表單常包含procedure頁面沒有寫清楚的站點結構表。若問題只是單純問「在哪裡下載」，看到的markdown連結本身就已經是答案，不需要額外呼叫get_form。
...
**範例3：get_page讀到頁面偵測到表單編號，問題需要完整流程細節時該進一步呼叫get_form**
情境：問題是「如何辦理休學」，get_page讀到procedure頁面全文，裡面出現「[偵測到表單編號: QP-T01-03-02]」標記，但頁面本身只寫「依說明至系所及各處室蓋章」，沒有列出具體是哪些處室。
正確做法：呼叫get_form(form_id="QP-T01-03-02")取得表單完整內容，因為表單本身通常有明確的蓋章站點列表，比procedure頁面的概略敘述更完整；讀完表單後才把逐一站點寫進最終答案。
錯誤做法：忽略[偵測到表單編號]標記，只憑procedure頁面「依說明至系所及各處室蓋章」這種模糊敘述就直接作答，讓答案少了具體站點資訊。"""
